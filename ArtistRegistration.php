...
<body>
	...
	<div class="contact" id="contact">
		<div class="container">
			<h3 class="wow fadeInUp animated" data-wow-delay=".5s">Artist SiginUp</h3>
			<div class="contact-info">
				...
				<div class="col-md-4 contact-grids wow fadeInRight animated" data-wow-delay=".7s">
					...
					<form>
						...
						<input type="text" name="location" class="textbox" placeholder="Location" required pattern="[A-Za-z-]+" title="enter characters only">
                ...
                <span >&nbsp;I Accept the Terms &amp; Conditions</span></td>
					...
					if (isset($_REQUEST['submit'])) {
						...
						$qryReg = "INSERT INTO `artist_registration` (`name`,gender,category,`district`,`location`,`phone`,`email`) values('$name','$gender','$category','$district','$location','$phone','$email')";
						$qryLog = "INSERT INTO login (uid,username,password,type,`status`)values((select max(aid)from artist_registration),'$email','$password','artist','0')";
						$qryPort = "INSERT INTO `artist_portfolio` (`aid`,`image`,`video`) values((select max(aid)from artist_registration),'noval','noval')";
						echo $qryReg . "<br>" . $qryLog;
						if ($conn->query($qryReg) == TRUE && $conn->query($qryLog) == TRUE && $conn->query($qryPort) == TRUE) {

							echo "<script>alert('Registration Request Sent Successfully');window.location='index.php'</script>";
						} else {
							echo "<script>alert('Registration failed');window.location='ArtistRegistration.php'</script>";
						}
					}
					?>
				</div>
